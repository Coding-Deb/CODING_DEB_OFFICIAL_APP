...
export default function Registered_Student({ navigation }) {
  ...
  useEffect(() => {
    async function getAllStudent() {
      try {
        const Sign_in_student = await axios.get("http://192.168.112.210:5000/dataapi/api_sign_in_data")
        setStudent(Sign_in_student.data)
      } catch (error) {
        console.log(error);
      }
    }
  ...
  const DeleteData = (id) => {
    axios.delete("http://192.168.112.210:5000/deleteapi/reg_student_delete/" + id)
      .then((res) => {
        console.log('1 Registered Student Deleted Successfully');
      })
    loaddata()
  }
  ...
  const loaddata = async () => {
    try {
      const student = await axios.get("http://192.168.112.210:5000/dataapi/api_sign_in_data")
          ...
          return (
            <View style={{ backgroundColor: 'white', height: 130, width: width - 30, marginTop: 30, marginBottom: 15, flexDirection: 'row', alignItems: 'center' }}>
              ...
              <View style={{ flexDirection: 'column' }}>
                <TouchableOpacity style={{ width: 120, height: 30, backgroundColor: '#c92a1e', justifyContent: 'center', alignItems: 'center' }}
